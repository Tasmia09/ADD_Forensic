...
def set_utid_date_map(image_name,ut_id, max_date):
    ...
    for row in mr.iterrows():
        ...
        if curr_date <= max_date:
            utid_dates_map[ut_id].append(curr_date)
    #mr = mr.loc[mr['date_only(YYYY-MM-DD)'].isin([max_date])]
    ...
    """for row in image_names_data.iterrows():
        ...
        if ids == ut_id:
            ...
            if curr_date <= max_date:
                utid_dates_map[ids].append(curr_date)"""
                
    
    value = utid_dates_map[ut_id]
...
def start():
    ...
    for row in image_names_data.iterrows():
        ...
        min_date = set_utid_date_map(image_name,utid,max_date)
    ...
    #utid_dates_map_df = pd.DataFrame.from_dict(utid_dates_map, orient='index')
    #utid_dates_map_df = pd.DataFrame(dict([ (k,pd.Series(v)) for k,v in utid_dates_map.items() ]))
    #utid_dates_map_df = utid_dates_map_df.T
    #utid_dates_map_df.to_csv('outputs/utid_dates_map(1).csv', encoding='utf-8', index=False)
    
    #utid_dates_map_string_df = pd.DataFrame.from_dict(utid_dates_map_string, orient='index')
    #utid_dates_map_string_df.to_csv('outputs/utid_dates_map_string.csv', encoding='utf-8', index=False)
...
def  ADD(image_name, utid,max_date,min_date):
    ...
    global match, donor_max_min_ADD,utid_dates_map_df, image_max_min_ADD
    for x in date_range:
